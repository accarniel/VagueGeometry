...
static LWGEOM* parse_gml(xmlNodePtr xnode, bool *hasz, int *root_srid) {
	...
	gml_lwerror("invalid GML representation of kernel or conjecture", 56);
...
/**
 * Ability to parse VGML geometry fragment and to return an VAGUEGEOM
 * or an error message.
 *
...
extern VAGUEGEOM *vaguegeom_from_vgml(const char *vgml, int *root_srid, uint8_t punion) {
	...
	xmlNodePtr xa;
	int xml_size = strlen(vgml);
	bool hasz=true;
	char *vg_type=NULL;
	
	/* Begin to Parse XML doc */
	xmlInitParser();
	xmldoc = xmlReadMemory(vgml, xml_size, NULL, NULL, XML_PARSE_SAX1);
	if (!xmldoc || (xmlroot = xmlDocGetRootElement(xmldoc)) == NULL) {
		xmlFreeDoc(xmldoc);
		xmlCleanupParser();
		gml_lwerror("invalid VGML representation", 1);
	}

	while (xmlroot != NULL && (xmlroot->type != XML_ELEMENT_NODE)) xmlroot = xmlroot->next;

	if (xmlroot == NULL) gml_lwerror("invalid VGML representation", 55);

	parse_gml_srs(xmlroot, &srs);
	if (*root_srid == SRID_UNKNOWN && srs.srid != SRID_UNKNOWN) {
		*root_srid = srs.srid;
	}
	...
	//	if (!is_gml_namespace(xa, false)) continue;
		...
		if (!strcmp((char *) xa->name, "Kernel")) {
			kernel = parse_gml(xa->children, &hasz, root_srid);
			if(kernel!=NULL) {
		...
		if (!strcmp((char *) xa->name, "Conjecture")) {
			conjecture = parse_gml(xa->children, &hasz, root_srid);
			if(conjecture!=NULL) {
				/*check if the kernel type is compatible with the vaguegeometry type */
	...
	if(kernel == NULL || conjecture == NULL) {
		if(vg_type != NULL) {
			if( strcasecmp( vg_type, "VaguePoint" )==0 ) {
				...
				gml_lwerror("Invalid VagueGeometry type", 4);
			}
		} else {
			gml_lwerror("Invalid VagueGeometry type, please a type must be setted", 4);
		}
	}
	...
	if ( root_srid != SRID_UNKNOWN ) {
		if(kernel!=NULL)
			kernel->srid = *root_srid;
		if(conjecture!=NULL)
			conjecture->srid = *root_srid;
	}
	 ...
	 */
	if(kernel!=NULL) {
		LWGEOM *tmp1 = lwgeom_force_2d(kernel);
		lwgeom_free(kernel);
		kernel = tmp1;
	} 
	if(conjecture!=NULL) {
		LWGEOM *tmp1 = lwgeom_force_2d(conjecture);
